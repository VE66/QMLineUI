...
//    self.textColor = [UIColor blackColor];
    self.textColor = [UIColor colorWithHexString:isDarkStyle ? QMColor_D4D4D4_text : QMColor_151515_text];
...
//    UIColor *commonLinkColor = [UIColor colorWithHexString:isDarkStyle ? @"#808FA6" : QMColor_News_Custom];
    ...
    for (CFIndex lineIndex = 0; lineIndex < numberOfLines; lineIndex++) {
        ...
        for (id glyphRun in (__bridge NSArray *)CTLineGetGlyphRuns(line)) {
            ...
            if (imageName) {
                ...
                NSString *imagePath = [self.customEmojiBundleName?:@"QMEmoticon.bundle" stringByAppendingPathComponent:imageName];
...
//    NSRegularExpression * const regexps[] = {kRobotRegularExpression(), kURLRegularExpression()};
    
    NSRegularExpression * const regexps[] = {kRobotRegularExpression(), kURLRegularExpression(), kPhoneNumerRegularExpression()};
    ...
    if (self.checkResults) {
        NSMutableDictionary *mutableLinkAttributes = [@{(NSString *)kCTUnderlineStyleAttributeName:@(NO)}mutableCopy];
        UIColor *commonLinkColor = self.checkColor;
        [mutableLinkAttributes setObject:commonLinkColor forKey:(NSString *)kCTForegroundColorAttributeName];
        [super addLinksWithTextCheckingResults:self.checkResults attributes:mutableLinkAttributes];
    }
}
        ...
        if (self.activeLink&&self.activeLink.result.resultType==NSTextCheckingTypeCorrection) {
            ...
            for (NSUInteger i=0; i<kURLActionCount; i++) {
                if ([result.replacementString hasPrefix:kURLActions[i]]) {
                    ...
                    if (result.range.location != NSNotFound && [kURLActions[i] isEqualToString:@"at->"]) {
                        // 找相同type不同位置的点击
                        NSString *regexStr = content;
                        if ([regexStr hasPrefix:@">"] && [regexStr hasSuffix:@"<"]) {
                            regexStr = [regexStr substringFromIndex:1];
                            regexStr = [regexStr substringToIndex:regexStr.length - 1];
                            NSString *sourceStr = [self.text substringToIndex:result.range.location];
                            NSRegularExpression *regular = [NSRegularExpression regularExpressionWithPattern:regexStr options:NSRegularExpressionCaseInsensitive error:nil];
                            NSArray *matchs = [regular matchesInString:sourceStr options:NSMatchingReportProgress range:NSMakeRange(0, sourceStr.length)];
                            content = [content stringByAppendingFormat:@"%ld",matchs.count];
                        }
                    }
