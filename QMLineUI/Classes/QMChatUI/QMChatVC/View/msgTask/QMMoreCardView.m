...
- (void)setSubViews {
    ...
    lab.textColor = [UIColor colorWithHexString:isDarkStyle ? QMColor_FFFFFF_text : QMColor_999999_text];
    ...
    self.moreTabView.backgroundColor = [UIColor colorWithHexString:isDarkStyle ? QMColor_Main_Bg_Dark : QMColor_Main_Bg_Light];
    ...
    [QMConnect sdkGetCommonDataWithParams:dict completion:^(id data) {
        ...
        if ([data[@"Succeed"] boolValue] == 1) {
            ...
            if (json && json.length > 0) {
                ...
                NSLog(@"dict = %@",dict);
                NSArray *arr = dict[@"data"][@"shop_list"];
                if (arr.count > 0) {
                    self.dataArr = arr;
...
- (instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier {
    if (self = [super initWithStyle:style reuseIdentifier:reuseIdentifier]) {
        ...
        self.iconImage.QMCornerRadius = 2;
        [self.contentView addSubview:self.iconImage];
        
        self.titleLabel = [[UILabel alloc] init];
        self.titleLabel.textColor = [UIColor colorWithHexString:isDarkStyle ? QMColor_FFFFFF_text : @"#000000"];
        ...
        self.statusLabel.font =[UIFont fontWithName:QM_PingFangSC_Reg size:14];
        self.statusLabel.textAlignment = NSTextAlignmentRight;
        [self.contentView addSubview:self.statusLabel];
        
        self.topLine = [[UIView alloc] init];
        self.topLine.backgroundColor = [UIColor colorWithHexString:isDarkStyle ? @"#373737" : @"#EBEBEB"];
        ...
        [self.titleLabel mas_makeConstraints:^(MASConstraintMaker *make) {
            ...
            make.right.greaterThanOrEqualTo(self.contentView).offset(-QMFixWidth(100));
            make.height.mas_equalTo(22);
        }];
        
        [self.statusLabel mas_makeConstraints:^(MASConstraintMaker *make) {
            make.right.equalTo(self.contentView).offset(-20);
            make.width.mas_equalTo(QMFixWidth(124)).priority(999);
...
- (void)setCellData:(QMCardInfoModel *)model {
    if (model.img.length > 0) {
        NSString *imgUrl = model.img;
        if ([imgUrl stringByRemovingPercentEncoding] == imgUrl) {
...
- (void)setupSubViews {
    ...
    self.iconImage = [[UIImageView alloc] init];
    self.iconImage.QMCornerRadius = 4;
    [self.contentView addSubview:self.iconImage];
    self.iconImage.backgroundColor = [UIColor colorWithHexString:@"#EBEBEB"];
    ...
    self.priceLabel.textColor = [UIColor colorWithHexString:isDarkStyle ? QMColor_999999_text : @"#262626"];;
    self.priceLabel.font = [UIFont fontWithName:QM_PingFangSC_Reg size:15];
    ...
    [self.titleLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.right.equalTo(self.contentView).offset(-QMFixWidth(84));
    ...
    [self.priceLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.top.equalTo(self.contentView).offset(12);
        make.width.mas_equalTo(QMFixWidth(60));
        make.height.mas_equalTo(18);
    }];
    
    [self.descLabel1 mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.equalTo(self.titleLabel);
        make.right.equalTo(self.contentView).offset(-QMFixWidth(72));
        make.height.mas_equalTo(18);
    ...
    [self.countLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        make.right.equalTo(self.contentView).offset(-16);
        make.top.equalTo(self.contentView).offset(34);
        make.width.mas_equalTo(QMFixWidth(40));
        make.height.mas_equalTo(18);
    }];
    ...
    [self.descLabel2 mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.equalTo(self.titleLabel);
        make.right.equalTo(self.contentView).offset(-QMFixWidth(89));
    ...
    [self.statusLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        make.right.equalTo(self.contentView).offset(-16);
        make.width.mas_equalTo(QMFixWidth(64));
        make.height.mas_equalTo(16);
        make.bottom.equalTo(self.contentView).offset(-12);
    }];
...
- (void)setCellData:(QMCardInfoModel *)model {
    ...
    if (model.attr_one.color.length > 0) {
        self.countLabel.textColor = [UIColor colorWithHexString:model.attr_one.color];
//        QMHEXSRGB(model.attr_one.color);
    } else {
        self.countLabel.textColor = QMHEXRGB(0x666666);
    }
    ...
    if (model.attr_two.color.length > 0) {
        self.statusLabel.textColor = [UIColor colorWithHexString:model.attr_two.color];
//        QMHEXSRGB(model.attr_two.color);
    } else {
        self.statusLabel.textColor = QMHEXRGB(0XFF6B6B);
    }
}
